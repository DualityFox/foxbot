...
module.exports = {
    run: (message, args, client) => {
        ...
        if(!target) return message.channel.send("Merci de mentionner un utilisateur").then(sent => sent.delete({timeout: 5e3}))
        let reason = args.slice(1).join(" ");
        if(!reason) return message.channel.send(`Merci de bien préciser la plainte pour pouvoir porter plainte contre un utilisateur **${target.user.tag}**`).then(sent => sent.delete({timeout: 5e3}))
